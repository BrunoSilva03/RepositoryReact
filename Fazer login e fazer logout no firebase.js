...
function App() {
  ...
  //false porque o usuário não começa logado no sistema
  const [user, setUser] = useState(false);
  //objeto vazio porque ele começa e não está logado né
  ...
  async function logarUsuario() {
    //auth é a conexão com nosso sistema de autenticação
    await signInWithEmailAndPassword(auth, email, senha)
    .then((value) => {
      //value são os detalhes do usuário logado
      console.log("USUÁRIO LOGADO COM SUCESSO NO SISTEMA!")
      console.log(value.user)

      //passando os atributos de uid e email  do usuário para a state
      //userDetail
      setUserDetail({
        uid: value.user.uid,
   ...
   async function fazerLogout() {
    //só preciso passar a conexão com a minha autenticação
    await signOut(auth)
    setUser(false);
    setUserDetail({});
